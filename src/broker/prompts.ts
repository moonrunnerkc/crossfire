...
export function buildFixPrompt(input: FixPromptInput): string {
  ...
  return [
    ...
    "1. Every repro command above exits non-zero after your change.",
    `2. The target's own test suite still passes: ${config.target.testCommand}`,
    "3. Every module you changed that has a sibling test file has a regression",
    "   case added to it, covering the shape you closed in the spellings that",
    "   shape can be written in, not only the one input the repro happens to use.",
    "The broker re-runs each repro and that suite itself, so a fix that does not",
    "flip its repro does not count, however well it reads. It cannot re-run a test",
    "you did not write: the repro proves this one input is dead, and the test is",
    "the only thing that keeps it dead once the repro is gone. A fix that lands",
    "without one is unfinished, and the next round has no way to tell.",
    "List those test files in files_changed alongside the source you edited.",
    "",
    "Constraints",
